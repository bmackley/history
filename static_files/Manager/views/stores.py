from django import forms
from django.conf import settings
from django.http import HttpResponse, HttpResponseRedirect, Http404
from Manager import models as m 
from . import templater

def process_request(request):
	'''Shows the List of Stores'''
	if not request.user.is_authenticated():
		return HttpResponseRedirect('/Manager/login')

	if request.urlparams[0] == 'delete':
		stores = m.stores.objects.get(id=request.urlparams[1])
		stores.active = False
		stores.save()

	if request.urlparams[0] == 'all':
		stores = m.stores.objects.all().order_by('locationName')
	else:
		stores = m.stores.objects.exclude(active=False).order_by('locationName')
	
	
	tvars = {
		'stores' : stores,
	}
	return templater.render_to_response(request, 'stores.html', tvars)



